...
from PySide6.QtWidgets import (
    ...
    QMessageBox, QGroupBox, QLineEdit, QDoubleSpinBox, QComboBox, QApplication, QAbstractItemView
)
...
class StepExhaust(QWidget):
    ...
    def _recompute_tuning(self) -> None:
        ...
        if self.state.engine_target_rpm:
            try:
                L_rec_mm = exhaust_quarter_wave_L_phys(float(self.state.engine_target_rpm), n_harm, D_m, T_exh) * 1000.0
            except Exception:
                L_rec_mm = None
        ...
        if a_exh and q_cfm is not None:
            self.lbl_tuning_status.setText(
                f"a_exh(T)={a_exh:.1f} m/s · Q_exh_peak={q_cfm:.1f} CFM · CSA={(csa_mm2 or 0):.0f} mm² · d_eq={(d_eq_mm or 0):.1f} mm"
            )
        else:
            self.lbl_tuning_status.setText("a_exh(T)=— m/s · Q_exh_peak=— CFM · CSA=— mm² · d_eq=— mm")
    ...
    def _save_to_state(self) -> None:
        ...
        for r in range(self.table.rowCount()):
            ...
            if lift in seen:
                ...
                seen[lift] = len(rows)
                rows.append(row)
        rows.sort(key=lambda x: x["lift_mm"])
        ...
        if intake and exhaust:
            ...
            if ei:
                ...
                if valid:
                    ...
                    self.lbl_alert.setText("ALERT: E/I poza zakresem 0.70–0.85" if (avg < 0.70 or avg > 0.85) else "")
