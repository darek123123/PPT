...
class StepExhaust(QWidget):
    ...
    def __init__(self, state: WizardState) -> None:  # noqa: PLR0915
        ...
        self.spn_L_mm.setToolTip("Długość runnera L (mm). Model ćwierćfali."); self.spn_L_mm.setValue(float(_pref("L_mm", 450.0)))
        ...
        self.spn_D_mm.setToolTip("Średnica rury D (mm). Używana do korekcji długości."); self.spn_D_mm.setValue(float(_pref("D_mm", 38.0)))
        row_D.addWidget(QLabel("D [mm]:", self)); row_D.addWidget(self.spn_D_mm)

        row_n = QHBoxLayout(); tuning_lay.addLayout(row_n)
        self.cmb_n_harm = QComboBox(self); self.cmb_n_harm.addItems(["1", "2", "3"])
        self.cmb_n_harm.setToolTip("Nieparzyste harmoniczne 1/3/5. 1⇒1., 2⇒3., 3⇒5.")
        n_saved = int(_pref("n_harm", 2)); self.cmb_n_harm.setCurrentIndex(max(0, min(2, n_saved - 1)))
        ...
        self.spn_T_exh_K.setToolTip("Szacowana temp. spalin w K."); self.spn_T_exh_K.setValue(float(_pref("T_exh_K", 700.0)))
        ...
        self.spn_v_target.setToolTip("Docelowa średnia prędkość w kolektorze."); self.spn_v_target.setValue(float(_pref("v_target_ms", 70.0)))
        row_v.addWidget(QLabel("v_target [m/s]:", self)); row_v.addWidget(self.spn_v_target)

        # Outputs
        self.lbl_rpm_for_L = QLabel("rpm dla L: —", self)
        self.lbl_L_rec = QLabel("L_rec: — mm", self); self.lbl_L_rec.setToolTip("Długość dla RPM target (ćwierćfala)")
        self.lbl_CSA = QLabel("CSA: — mm²", self); self.lbl_CSA.setToolTip("Wymagane CSA kolektora")
        self.lbl_d_eq = QLabel("d_eq: — mm", self)
        for w in (self.lbl_rpm_for_L, self.lbl_L_rec, self.lbl_CSA, self.lbl_d_eq): tuning_lay.addWidget(w)

        self.lbl_tuning_status = QLabel("", self)
        self.lbl_corner_notice = QLabel("", self); self.lbl_corner_notice.setStyleSheet("color:#aa8800;font-style:italic;")
        tuning_lay.addWidget(self.lbl_tuning_status); tuning_lay.addWidget(self.lbl_corner_notice)

        # Primary length helper
        helper = QGroupBox("Długość primary (1D)", self); helper.setToolTip("Szacunek z modelu ćwierćfali")
        ...
        hl.addWidget(self.lbl_len_exh); hl.addStretch(1); tuning_lay.addWidget(helper)
        ...
        self.plot_ei = MplCanvas(); self.plot_ei.set_readout_units("mm", "-")
        ...
        def estimate_q_peaks() -> tuple[float, str]:
            try:
                ...
                result = run_all(session, dp_ref_inH2O=self.state.air_dp_ref_inH2O or 28.0, engine_v_target=(self.state.engine_target_rpm or 100.0))
        ...
        def recompute() -> None:
            ...
            L_mm = float(self.spn_L_mm.value()); D_mm = float(self.spn_D_mm.value()); n_harm = int(self.cmb_n_harm.currentText())
            T_exh = float(self.spn_T_exh_K.value()); v_target = float(self.spn_v_target.value()); D_m = D_mm / 1000.0
            try: rpm_for_L = exhaust_quarter_wave_rpm_for_L(L_mm, n_harm, D_m, T_exh)
            except Exception: rpm_for_L = None
            L_rec_mm = None; target_rpm = self.state.engine_target_rpm
            if target_rpm:
                try: L_rec_mm = exhaust_quarter_wave_L_phys(float(target_rpm), n_harm, D_m, T_exh) * 1000.0
                except Exception: L_rec_mm = None
            q_peak, notice = estimate_q_peaks(); csa_mm2 = None; d_eq_mm = None
            if q_peak > 0 and v_target > 0:
                try:
                    csa_m2, csa_mm2_val = collector_csa_from_q(q_peak, v_target)
                    csa_mm2 = csa_mm2_val; d_eq_mm = (4.0 * csa_m2 / 3.141592653589793) ** 0.5 * 1000.0
                except Exception: pass
            elif notice == "": notice = "Brak danych – pominięto CSA"
            ...
            except Exception: a_exh = None
            q_cfm = F.m3s_to_cfm(q_peak) if q_peak > 0 else None
            self.lbl_tuning_status.setText(
                f"a_exh(T)={(a_exh or 0):.1f} m/s · Q_exh_peak={(q_cfm or 0):.1f} CFM · CSA={(csa_mm2 or 0):.0f} mm² · d_eq={(d_eq_mm or 0):.1f} mm" if a_exh and q_cfm is not None else "a_exh(T)=— m/s · Q_exh_peak=— CFM · CSA=— mm² · d_eq=— mm"
        ...
        # Wiring
        def on_any_change():
            try: recompute(); compute_primary_length()
            except Exception: pass

        for w in (self.spn_L_mm, self.spn_D_mm, self.spn_T_exh_K, self.spn_v_target): w.valueChanged.connect(lambda *_: on_any_change())
        ...
        # Initial populate
        self._load_from_state(); self._update_counts(); self._emit_valid(); recompute(); compute_primary_length(); self._update_csa_numbers()
    ...
    def eventFilter(self, obj, event):  # type: ignore[override]
        if obj is self.table.viewport() and event.type() == QEvent.KeyPress:
            ...
            if ke.matches(QKeySequence.Paste):
                self._paste_from_clipboard(); return True
        return super().eventFilter(obj, event)

    def _load_from_state(self) -> None:
        rows = self.state.measure_exhaust; self.table.blockSignals(True); self.table.setRowCount(len(rows))
        for r, row in enumerate(rows):
            for c, key in enumerate(["lift_mm", "q_cfm", "dp_inH2O", "swirl_rpm"]):
                val = row.get(key); self.table.setItem(r, c, QTableWidgetItem("" if val is None else str(val)))
    ...
    def _save_to_state(self) -> None:
        ...
        rows: List[Dict[str, Any]] = []; seen: Dict[float, int] = {}
        for r in range(self.table.rowCount()):
            lift = parse_item(self.table.item(r, 0)); q = parse_item(self.table.item(r, 1)); dp = parse_item(self.table.item(r, 2)); swirl = parse_item(self.table.item(r, 3))
            if lift is None or q is None: continue
            lift = round(max(lift, 0.0), 3); row: Dict[str, Any] = {"lift_mm": lift, "q_cfm": max(q, 0.0)}
            if dp is not None and dp > 0: row["dp_inH2O"] = dp
            if swirl is not None and swirl >= 0: row["swirl_rpm"] = swirl
            if lift in seen: rows[seen[lift]] = row
            else: seen[lift] = len(rows); rows.append(row)
        rows.sort(key=lambda x: x["lift_mm"]); self.state.measure_exhaust = rows

    def _update_counts(self) -> None:
        rows = self.state.measure_exhaust; n = len(rows); m = sum(1 for r in rows if r.get("dp_inH2O") is not None); k = sum(1 for r in rows if r.get("swirl_rpm") is not None)
        self.lbl_counts.setText(f"n: {n}, z dp: {m}, ze swirl: {k}")

    def _on_changed(self, *_: Any) -> None:
        self._save_to_state(); self._update_counts(); self._emit_valid()

    def _paste_from_clipboard(self) -> None:
        from PySide6.QtWidgets import QApplication
        txt = QApplication.clipboard().text(); rows = parse_rows(txt or "");
        if not rows: return
        start = self.table.rowCount(); self.table.setRowCount(start + len(rows))
        for i, (lift, q, dp, swirl) in enumerate(rows):
            r = start + i; vals = [lift, q, dp if dp is not None else "", swirl if swirl is not None else ""]
    ...
    def _autofill(self) -> None:
        ...
        if not plan:
            QMessageBox.information(self, "Plan EXH", "Brak planu dla EXH."); return
        rows_map: Dict[float, Dict[str, Any]] = {(row.get("lift_mm") or 0.0): dict(row) for row in self.state.measure_exhaust}
        for lift in plan:
            if lift not in rows_map: rows_map[lift] = {"lift_mm": lift}
    ...
    def _clear(self) -> None:
        self.state.measure_exhaust = []; self._load_from_state(); self._update_counts(); self._emit_valid()

    def _emit_valid(self) -> None:
        self.sig_valid_changed.emit(True)

    def _compute(self) -> None:
        try: session = self.state.build_session_for_run_all()
        except Exception: return
        result = run_all(session, dp_ref_inH2O=self.state.air_dp_ref_inH2O or 28.0, a_ref_mode="eff", eff_mode="smoothmin", engine_v_target=(self.state.engine_target_rpm or 100.0))
        series = result.get("series", {}); intake: List[Dict[str, Any]] = series.get("intake", [])  # type: ignore[assignment]
        exhaust: List[Dict[str, Any]] = series.get("exhaust", [])  # type: ignore[assignment]
        ei = series.get("ei", [])
        self.plot_ei.clear()
        if ei:
            lifts_m = [float(row.get("lift_m") or 0.0) for row in ei]; lifts_mm = [v * 1000.0 for v in lifts_m]
            vals = [row.get("EI") for row in ei]; valid_vals = [float(v) for v in vals if v is not None]
            title_extra = f" · mean={sum(valid_vals)/len(valid_vals):.3f}" if valid_vals else ""
            if lifts_mm and any(v is not None for v in vals):
                self.plot_ei.plot_xy(lifts_mm, [float(v) if v is not None else 0.0 for v in vals], label="E/I", xlabel="Lift [mm]", ylabel="E/I [–]", title=f"E/I vs Lift{title_extra}")
        self.plot_ei.render()
        if intake and exhaust:
            ...
            if ei:
                ...
                if valid:
                    avg = sum(valid) / len(valid); txt += f"; mean(E/I)={avg:.3f}"; self.lbl_alert.setText("ALERT: E/I poza zakresem 0.70–0.85" if (avg < 0.70 or avg > 0.85) else "")
            self.lbl_corner.setText("")
        else:
            txt = "Brak danych exhaust — E/I będzie puste"; self.lbl_alert.setText(""); self.lbl_corner.setText("Brak danych wydechu — wykres niedostępny (INFO)")
        self.lbl_ei.setText(txt); self._update_csa_numbers(result); self._update_primary_length()

    def _update_csa_numbers(self, result: Optional[Dict[str, Any]] = None) -> None:
        try:
            v_target = float((self.ed_v_exh.text() or "70").replace(",", ".")); assert v_target > 0
        except Exception:
            self.lbl_A_req.setText("A_req = — mm²"); self.lbl_d_eq.setText("d_eq = — mm"); return
        if result is None:
            try:
                session = self.state.build_session_for_run_all(); result = run_all(session, dp_ref_inH2O=self.state.air_dp_ref_inH2O or 28.0, a_ref_mode="eff", eff_mode="smoothmin", engine_v_target=(self.state.engine_target_rpm or 100.0))
            except Exception: result = None
        q_peak = 0.0
        try:
            ex = (result or {}).get("series", {}).get("exhaust", [])  # type: ignore[union-attr]
            if ex: q_peak = max(float(r.get("q_m3s_ref") or 0.0) for r in ex)
        except Exception: q_peak = 0.0
        if q_peak > 0.0:
            try:
                A_req = F.header_csa_required(q_peak, v_target); A_mm2 = A_req * 1e6; d_eq = (4.0 * A_req / 3.141592653589793) ** 0.5 * 1000.0
                self.lbl_A_req.setText(f"A_req = {A_mm2:.0f} mm²"); self.lbl_d_eq.setText(f"d_eq = {d_eq:.1f} mm"); self.lbl_d_eq2.setText(f"d_eq = {d_eq:.1f} mm")
            except Exception:
                self.lbl_A_req.setText("A_req = — mm²"); self.lbl_d_eq.setText("d_eq = — mm")
        else:
            self.lbl_A_req.setText("A_req = — mm²"); self.lbl_d_eq.setText("d_eq = — mm")

    def _update_primary_length(self) -> None:
        try:
            phi = float((self.ed_phi_exh.text() or "90").replace(",", ".")); harm = int(float((self.ed_harm_exh.text() or "1").replace(",", ".")))
            rpm = float((self.ed_rpm_exh.text() or "6500").replace(",", ".")); T = float(self.state.air.T if self.state.air is not None else 293.15)
            a_T = F.speed_of_sound(T); L_m = F.primary_length_exhaust_quarterwave(rpm, T, phi_deg=phi, harmonic=harm)
